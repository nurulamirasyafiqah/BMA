...
5. On first run this creates `pb_data/data.db` (the SQLite file) and applies
   ...
   - starter site settings, services, and news rows for both India and
     ...
     this India zip only ever shows/edits the India rows via
     `SITE_COUNTRY`; the Vietnam rows just sit unused in this database
...
**Also setting up the Vietnam site?** That's the matching Vietnam zip —
...
This zip's `config.php` already defaults `SITE_COUNTRY` to `india`, and
...
`SITE_COUNTRY=vietnam`) for local testing purposes; doing that on a real
India deployment would be unusual since this zip's database only really
has India's *real* content in it (see §4 for why there are two zips).

Then serve the PHP folder as usual, e.g.:
```
PB_URL=http://127.0.0.1:8090 SITE_COUNTRY=india php -S localhost:8000
```
...
## 4. This is the INDIA website — a fully independent system

This zip is **one of two separate zips** for this project — this one runs
the India website; a matching Vietnam zip runs the Vietnam website. They
are **completely independent systems**, each with its own PocketBase
database, its own domain, and its own admin logins:

- An admin account created here (on the India site) does not exist on the
  Vietnam site, and vice versa — they're different databases entirely, not
  just different accounts in a shared one.
...
`SITE_COUNTRY` setting (already set to `india` in this zip's
...
If you want one person to be able to manage *both* the India and Vietnam
...
`bidmyasset-india.com/admin/login.php` to manage India, and separately
into `bidmyasset-vietnam.com/admin/login.php` to manage Vietnam — same
...
exists with the **same credentials** in both this zip and the Vietnam
...
change it on the Vietnam site — you have to do it in both places).
...
2. From this project folder, run:
   ...
   ```
3. Open:
   - `http://localhost:8000/` — the India website
   - `http://localhost:8000/admin/login.php` — the India admin panel
...
**Running the Vietnam zip on the same machine too?** Put it in a
...
to change its ports (e.g. `8002:80` and `8091:8090` instead of `8000:80`
and `8090:8090`) so they don't clash with this India stack's ports. The
Vietnam zip's `docker-compose.yml` has a comment explaining exactly where
to change this.
...
3. Point a reverse proxy (Caddy or Nginx with Let's Encrypt) at this
   container's port 8000 for HTTPS, using your India domain
   (e.g. `bidmyasset.in`).
...
6. Back up the `pb_data` Docker volume regularly — that's this site's
   entire database, independent from the Vietnam site's.

The Vietnam zip gets deployed the same way, either on this same server
...
the Vietnam zip to keep both sites on the same PocketBase version.
