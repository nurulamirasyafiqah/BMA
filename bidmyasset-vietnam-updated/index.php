...
$auction_images = array_values(array_filter(
    get_auction_images($country),
    function ($img) { return PocketBase::fileUrl($img, 'image', 'auction_images') !== null; }
));
...
<section class="auction" id="auction">
  <div class="container auction-inner">
    <div class="auction-img auction-carousel" data-carousel>
      <?php if ($auction_images): ?>
        <div class="carousel-track">
          <?php foreach ($auction_images as $i => $img): ?>
            <div class="carousel-slide<?= $i === 0 ? ' active' : '' ?>">
              <img src="<?= h(PocketBase::fileUrl($img, 'image', 'auction_images')) ?>" alt="Used industrial machinery at auction<?= !empty($img['caption']) ? ' - ' . h($img['caption']) : '' ?>">
              <?php if (!empty($img['caption'])): ?>
                <div class="carousel-caption"><?= nl2br(h($img['caption'])) ?></div>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
        </div>
        <?php if (count($auction_images) > 1): ?>
          ...
          <div class="carousel-dots">
            <?php foreach ($auction_images as $i => $img): ?>
              <button type="button" class="dot<?= $i === 0 ? ' active' : '' ?>" data-index="<?= $i ?>" aria-label="Go to image <?= $i + 1 ?>"></button>
            <?php endforeach; ?>
          </div>
        <?php endif; ?>
      <?php else: ?>
...
<!-- BANNER STRIP: before/after comparison slider -->
<div class="compare-slider" data-compare>
  <div class="compare-img compare-after">
    <img src="<?= h(pb_image($settings, 'factory_image_2', 'site_settings', ($base_path ?? '') . 'assets/placeholder.svg')) ?>" alt="After: empty, cleared factory floor">
  </div>
  <div class="compare-img compare-before" style="clip-path:inset(0 50% 0 0);">
    <img src="<?= h(pb_image($settings, 'factory_image_1', 'site_settings', ($base_path ?? '') . 'assets/placeholder.svg')) ?>" alt="Before: factory floor with idle machinery still in place">
  </div>
  <div class="compare-handle" style="left:50%;">
    <span class="compare-handle-btn">
      <svg viewBox="0 0 24 24" width="9" height="13" fill="none" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 6 9 12 15 18"/></svg>
      <svg viewBox="0 0 24 24" width="9" height="13" fill="none" stroke="#111" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 6 15 12 9 18"/></svg>
    </span>
  </div>
</div>
...
<section class="listed" id="machinery">
  <div class="container">
    ...
    <div class="listed-slider-wrap">
      <button type="button" class="listed-slider-arrow listed-slider-prev" aria-label="Previous machinery">&#10094;</button>
      <div class="listed-slider" data-listed-slider>
      ...
      <button type="button" class="listed-slider-arrow listed-slider-next" aria-label="Next machinery">&#10095;</button>
    </div>
    <div class="listed-dots" data-listed-dots></div>
  </div>
</section>
  ...
  <div class="container">
    ...
    <div class="news-slider-wrap">
      <div class="news-slider" data-news-slider>
        ...
        <div class="news-card">
          <h4><a href="news-article.php?id=<?= h($item['id']) ?>"><?= h($item['title']) ?></a></h4>
    ...
    <div class="news-more">
      <a class="btn btn-outline" href="news.php">View All News</a>
    </div>
  </div>
</section>
